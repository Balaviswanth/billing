	...
	elseif($for=='2'){
		...
		while($row=mysqli_fetch_array($res)){
			...
			$str.="	<tr>
						...
						<td>".round($row['paid'],2)."</td>
						<td>".round($row['discount'],2)."</td>
...
select 
		...
		pur_qty,
		cost,
		product.qty as avail_qty,
		in_business,
		product.qty*selling_price as sellable 
		from product left join  
			(select 
             	product_id,
             	max(COALESCE(price,0)) as max_sales,
             	min(COALESCE(price,0)) as min_sales,
             	avg(COALESCE(price,0)) as avg_sales,
             	sum(COALESCE(sales.qty,0)) as sales_qty,
             	sum(COALESCE(price,0)*COALESCE(sales.qty,0)) as sales_amt 
             from sales group by product_id) as sales 
		on product.product_id=sales.product_id
		left join
			(select purchase.product_id, 
             	sum(purchase_qty) as pur_qty,
             	sum(purchase_price*purchase_qty) as cost,
				
             	product.qty*max(purchase_price) as in_business
             	
			from purchase join product on purchase.product_id=product.product_id group by product_id) as purchase
